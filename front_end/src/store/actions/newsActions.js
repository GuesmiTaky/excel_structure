...
export const listNewsAction = (currentPage, perPage) => async (dispatch) => {
  try {
    ...
    const response = await axiosClient.get(
      "/news?page=" + currentPage + "&per_page=" + perPage
    );
...
export const createNewsAction = (news) => async (dispatch) => {
  try {
    dispatch({
      type: NEWS_CREATE_REQUEST,
    });
    const response = await axiosClient.post(`/news`, news);
...
//function update
export const updateNewsAction = (id, news) => async (dispatch) => {
  try {
    dispatch({
      type: NEWS_UPDATE_REQUEST,
    });
    const response = await axiosClient.put(`/news/${id}`, news);
    if (response.status === 200) {
      dispatch({
