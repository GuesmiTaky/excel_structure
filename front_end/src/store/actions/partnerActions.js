...
export const listPartnerAction = (currentPage, perPage) => async (dispatch) => {
  try {
    ...
    const response = await axiosClient.get(
      "/partner?page=" + currentPage + "&per_page=" + perPage
    );
...
export const createPartnerAction = (partner) => async (dispatch) => {
  try {
    dispatch({
      type: PARTNER_CREATE_REQUEST,
    });
    const response = await axiosClient.post(`/partner`, partner);
...
//function update
export const updatePartnerAction = (id, partner) => async (dispatch) => {
  try {
    dispatch({
      type: PARTNER_UPDATE_REQUEST,
    });
    const response = await axiosClient.put(`/partner/${id}`, partner);
    if (response.status === 200) {
      dispatch({
